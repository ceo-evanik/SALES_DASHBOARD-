// backend/controllers/text.controller.js
...
export const createText = async (req, res, next) => {
  try {
    ...
    logger.info(`Admin(${req.user._id}) created mission text`);
    res.status(201).json({ success: true, message: "Mission text created", data: text });
  } catch (err) {
    next(err);
  }
};
...
export const getTextById = async (req, res, next) => {
  try {
    const text = await Text.findById(req.params.id).populate("createdBy", "name email");
    if (!text) {
      return res.status(404).json({ success: false, message: "Mission text not found" });
    }
...
// -------------------- Update Mission Text (Admin/Superadmin Only) --------------------
export const updateText = async (req, res, next) => {
  try {
    ...
    logger.info(`Admin(${req.user._id}) updated mission text`);
    res.json({ success: true, message: "Mission text updated", data: text });
  } catch (err) {
    next(err);
  }
};

// -------------------- Delete Mission Text (Admin/Superadmin Only) --------------------
export const deleteText = async (req, res, next) => {
  try {
    ...
    logger.info(`Admin(${req.user._id}) deleted mission text`);
