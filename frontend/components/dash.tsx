...
//   const [remainingWorkingDaysInMonth, setRemainingWorkingDaysInMonth] =
//     useState<number>(0);
...
//           // ✅ Mission Target & End Date ko API se uthao
//           const targetAmount = data.data?.Mission?.targetAmount || 0;
//           const targetEndDate = data.data?.Mission?.endDate
//             ? new Date(data.data.Mission.endDate)
//             : null;
...
//                 Mission: {formatINR(summary?.Mission?.targetAmount)} by{" "}
//                 {summary?.Mission?.endDate
//                   ? new Date(summary.Mission.endDate).toLocaleDateString(
//                       "en-US",
//                       {
//                         month: "long",
//                         day: "numeric",
...
//                   {weeksRemaining} Weeks to Excellence
//                 </span>
//               </p>
//             </div>
//           </div>
...
//                       ? Math.round(summary.Total.percent)
//                       : "-"}
//                     %
//                   </p>
//                   <p className="text-sm text-gray-600 dark:text-gray-300 mt-2 tracking-wide font-medium">
...
//                       {formatINR(summary?.Acquisition?.target)}
//                     </td>
//                     <td className="p-2 truncate">
//                       {formatINR(summary?.Acquisition?.achieved)}
//                     </td>
//                     <td className="p-2 truncate">
//                       {summary?.Acquisition?.percent?.toFixed(1) || "-"}%
//                     </td>
//                     <td className="p-2 truncate">
//                       {formatINR(summary?.Total?.target)}
//                     </td>
    ...
    <div className="flex-1 p-4 md:p-6 space-y-6 font-sans dark:bg-gray-900">
      ...
      <Card className="bg-gradient-to-r from-indigo-600 via-purple-600 to-blue-600 text-white shadow-xl rounded-2xl">
        <CardHeader className="flex flex-col md:flex-row justify-between items-start md:items-center space-y-4 md:space-y-0">
          ...
          <div className="text-right text-sm md:text-base space-y-1 text-gray-100">
            <p>{todayDate}</p>
            <p>
              Working Days Remaining:{" "}
      ...
      <Card className="shadow-md rounded-2xl">
        <CardHeader>
          <CardTitle className="text-lg md:text-xl font-semibold text-gray-800 dark:text-gray-200">
            Performance Overview
          </CardTitle>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {/* Total Achieved */}
              <StatCard
                value={formatINR(summary?.Total?.achieved)}
                label="TOTAL ACHIEVED"
                gradient="from-indigo-200 via-blue-100 to-purple-50 dark:from-indigo-900 dark:via-blue-800 dark:to-purple-900"
              ...
              <StatCard
                ...
                gradient="from-green-200 via-lime-100 to-teal-50 dark:from-green-900 dark:via-lime-800 dark:to-teal-900"
              ...
              <StatCard
                ...
                gradient="from-yellow-200 via-amber-100 to-orange-50 dark:from-yellow-900 dark:via-amber-800 dark:to-orange-900"
                textColor="text-yellow-900 dark:text-yellow-200"
              />

              {/* Required Daily Rate */}
              <StatCard
                value={formatINR(summary?.RequiredRate)}
                label="REQUIRED DAILY RATE"
                gradient="from-red-200 via-rose-100 to-pink-50 dark:from-red-900 dark:via-rose-800 dark:to-pink-900"
      ...
      <Card className="rounded-2xl">
        <CardHeader className="bg-gradient-to-r from-indigo-600 via-purple-600 to-blue-600 text-white rounded-t-2xl p-4 flex flex-col md:flex-row justify-between items-start md:items-center">
          {/* Legend */}
          <div className="flex flex-col md:flex-row gap-3">
            <LegendDot label="RENEWAL" />
            <LegendDot label="ACQUISITIONS" />
          </div>

          {/* Icons Legend */}
          <div className="flex flex-wrap gap-4 text-xs">
            <LegendIcon icon={<Target className="h-4 w-4 text-yellow-200" />} label="TOTAL TARGET" />
            <LegendIcon icon={<BarChart3 className="h-4 w-4 text-green-200" />} label="TOTAL ACHIEVED" />
            <LegendIcon icon={<Percent className="h-4 w-4 text-red-200" />} label="% ACHIEVEMENT" />
          </div>
        </CardHeader>
              ...
              <table className="w-full text-left text-sm min-w-[800px] border-collapse">
                <thead className="bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 uppercase text-xs">
                  <tr>
                    ...
                    <th className="p-2">%</th>
                    <th className="p-2">TOTAL TARGET</th>
                    <th className="p-2">TOTAL ACHIEVED</th>
                    <th className="p-2">%ACHIEVEMENT</th>
                    <th className="p-2">BALANCE</th>
                    <th className="p-2">CURRENT AVG</th>
                    <th className="p-2">REQUIRED RATE</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200 dark:divide-gray-700 text-gray-800 dark:text-gray-200">
                  <tr className="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                    <td className="p-2">{formatINR(summary?.Acquisition?.target)}</td>
                    <td className="p-2">{formatINR(summary?.Acquisition?.achieved)}</td>
                    <td className="p-2">{summary?.Acquisition?.percent?.toFixed(1) || "-"}%</td>
                    <td className="p-2">{formatINR(summary?.Total?.target)}</td>
                    <td className="p-2 text-green-600 dark:text-green-400">{formatINR(summary?.Total?.achieved)}</td>
                    <td className="p-2 text-red-600 dark:text-red-400">{summary?.Total?.percent?.toFixed(1) || "-"}%</td>
                    <td className="p-2">{formatINR(summary?.Balance)}</td>
                    <td className="p-2">{formatINR(summary?.CurrentAvg)}</td>
                    <td className="p-2 text-green-600 dark:text-green-400">{formatINR(summary?.RequiredRate)}</td>
                  </tr>
                </tbody>
              </table>
            </div>
...
function StatCard({
  ...
  return (
    <Card
      className={`bg-gradient-to-br ${gradient} border rounded-2xl shadow-md hover:shadow-lg transition-all`}
    >
      <CardContent className="p-6">
        <p className={`text-2xl md:text-3xl font-extrabold ${textColor}`}>
          {value}
        </p>
        <p className="text-sm text-gray-600 dark:text-gray-300 mt-2 font-medium tracking-wide">
          {label}
        </p>
      </CardContent>
    </Card>
